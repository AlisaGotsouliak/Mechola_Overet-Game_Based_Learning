...
namespace template.Server.Helpers
{
	public class AuthRepository
	{
        ...
        public async Task<KeyValuePair<string, string>> CheckAuth(string currentDomain)
        {
            ...
            {
                isProd = false;
                AuthUser devUser = new AuthUser() { FirstName = "בדיקה", LastName = "פורטלמ", PortelemId = -1 };

                int devUserId = await loginFunc(devUser, isProd);
                if (devUserId == 0)
            ...
            int userId = await loginFunc(user, isProd);
            if (userId == 0)
        ...
        private async Task<int> loginFunc(AuthUser user, bool isProduction)
        {
            object getParam = new
            {
            ...
            if (userId == 0)
            {
                ...
                userId = await _db.InsertReturnIdAsyncGames(insertQuery, user);
                if (userId == 0)
                    return 0;
            }
